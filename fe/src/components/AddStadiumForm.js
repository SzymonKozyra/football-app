...
const AddStadiumForm = () => {
    ...
    const [duplicateStadiums, setDuplicateStadiums] = useState([]); // For duplicate stadiums
            ...
            axios.post('http://localhost:8080/api/stadiums/import', formData, {
                ...
                .then(response => {
                    ...
                    if (response.data.duplicates && response.data.duplicates.length > 0) {
                        setDuplicateStadiums(response.data.duplicates); // Set duplicate stadiums
                    } else {
                        setDuplicateStadiums([]); // Clear duplicates if none
                    }
    ...
    return (
        <div>
            ...
            {/* Display the duplicate stadiums after import */}
            {duplicateStadiums.length > 0 && (
                <div>
                    <h3>Duplicate Stadiums</h3>
                    <ul>
                        {duplicateStadiums.map((stadium, index) => (
                            <li key={index}>
                                {stadium.name} in {stadium.cityName}, {stadium.countryName}
                            </li>
                        ))}
                    </ul>
                </div>
